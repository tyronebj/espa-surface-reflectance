...
C       real fac,pi
       ...
       character*80 err_msg
       integer retval
       data lambda/0.443,0.490,0.560,0.665,0.705,0.740,0.783,
     s  0.842,0.865,0.945,1.375,1.61,2.19,4.0,4.0,4.0/ 
        real eps
        real mraot550nm
        integer iverbose
C       pi=acos(0.)*2.
...
c       write(6,*) "Details roatm ",xts,xtv,xfi,raot550nm,pres,ib,roatm
c       endif
...
c accuracy 2e-07... for 0<xtau<1
     ...
     c                  0.05519968,-0.00976004,0.00107857/
      ...
      return
        ...
        subroutine local_chand (xphi,xmuv,xmus,xtau,xrray)
...
c constant : xdep: depolarization factor (0.0279)
        ...
        real xphi,xmus,xmuv,xtau,xrray,phios,xcosf1,xcosf2
...
c       deltaaot=deltaaot/(aot550nm(iaot2)-aot550nm(iaot1))
     ...
     s       roatm,err_msg,retval)
       parameter (fac = 0.017453293)
C       real pi
       real nbfic(22,20),tts(22),xtsmin,xtsstep,xtvmin,xtvstep
       real rolutt(16,7,22,8000),nbfi(22,20),tsmax(22,20),tsmin(22,20)
       real ttv(22,20)
       integer indts(22)
       real xts,xtv,xfi,xmus,xmuv,scaa,xtsmax
       real aot550nm(22),raot550nm       
       integer its,itv,iaot,iaot2
       integer ip1,ip2,ip
       real rop1,rop2
       real tpres(7)
       real pres,dpres
...
!             sca2=tsmax(its,itv)-isca*4.0
             ...
             sca2=xtsmax-isca*4.0
              ...
              isca=int(nbfi(its,itv))-1
!             sca1=tsmax(its,itv)-(isca-1)*4.0
             sca1=xtsmax-(isca-1)*4.0
             sca2=tsmin(its,itv)
             endif
!           iindex=indts(its)+nbfic(its,itv)-nbfi(its,itv)+isca
           iindex=indts(its)+int(nbfic1)-int(nbfi1)+isca
C           
...
!           iindex=indts(its)+nbfic(its,itv)-nbfi(its,itv)+1
           iindex=indts(its)+int(nbfic1)-int(nbfi1)+1
           roinf=rolutt(ib,ip1,iaot1,iindex)
...
!           iindex=indts(its+1)+nbfic(its+1,itv)-nbfi(its+1,itv)+1
           iindex=indts(its+1)+int(nbfic2)-int(nbfi2)+1
           roinf=rolutt(ib,ip1,iaot1,iindex)
!           rosup=rolutt(ib,ip1,iaot1,iindex)
...
!           iindex=indts(its)+nbfic(its,itv+1)-nbfi(its,itv+1)+1
           iindex=indts(its)+int(nbfic3)-int(nbfi3)+1
           roinf=rolutt(ib,ip1,iaot1,iindex)
!           rosup=rolutt(ib,ip1,iaot1,iindex)
...
c     S  ,rop2
       ...
       subroutine readlutsmsi(CAMOD,iendarg,tauray,oztransa,wvtransa,
...
c       if (bandid.ne.sbandname(i)) then
c       write(err_msg,*) "Error on rayleigh o.d file,"//
C     s     " check tauray-modis.ASC"
c     s     " check ", tauraynm//char(0)
c       retval=1
 101   Format(A7,1X,F10.5)       
c       return
c       endif
       enddo
       close(1)
...
C       Open(1,file="gascoef-modis.ASC")
       ...
       Read(1,*) (oztransa(i),i=1,13)
       Read(1,*) (wvtransa(i),i=1,13)
       Read(1,*) (wvtransb(i),i=1,13)
       Read(1,*) (ogtransa1(i),i=1,13)
       Read(1,*) (ogtransb0(i),i=1,13)
       Read(1,*) (ogtransb1(i),i=1,13)
       close(1)
